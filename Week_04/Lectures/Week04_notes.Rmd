...
Quiz Question:
In the penguin dataset, I only want to keep data from the year 2007. Which set of code is correct?
...
Quiz Question:
I have a dataframe with columns Site, Temperature, and Treatment and the dataframe is called TempData. The data are currently in degrees C, but I want to add a new column that converts the temperature to Kelvin. Which of the following is the correct way to do it?
TempData <- TempData %>% mutate(Temp_k = Temperature + 273.1)
...
```
Quiz Question:
For the dataframe from the prior question (TempData with columns Site, Temperature, and Treatment), I now want to calculate the mean Temperature by Site. Which is the correct way to do this?

TempData_summary<-TempData %>% group_by(Site)%>% summarise(meanTemp = mean(Temperature, na.rm = TRUE)
...
Quiz Question:
If I have data in the format pictured below, and I want to rearrange the data such that I have one column with all the parameter names for each variable (i.e. salinity, NN, pH, etc.) and one column with all the values while retaining all the appropriate metadata (i.e. Site, Zone, etc). What function do I use?
pivot_longer()
...
#ggsave(here("Week_04", "Output","Week04_homework_02.png"),width=7, height=5)  
```
