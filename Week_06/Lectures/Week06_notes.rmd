...
* can use multiple languages in the exact same script
* feed in code to Quarto document and it will convert to HTML, etc.
...
- Can code in this chunk in whatever language you want.  
  - write python in the {} if you want to use this language
...
* in Quarto, we go into the code chunk and use #| to tell it we are using YAML. 
* put #| warning: false  
  * supress warnings (won't show up in render but will in the console). 
  * can put it in the beginning (default) and it will do it for the whole document. 
* use echo or eval to hide code or show it but not run it. 
* loading libraries you can set message: false so that it doesn't show all messages  
* fig- to control information about figures inside code chunks. 
...
beep(5)
beep(9)
* can use to tell when code is done running
