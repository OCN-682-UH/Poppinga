...
scoobydoo_good <- tt$scoobydoo %>% as_tibble() # get scooby doo data frame
...
true<-function(x) as.character(x) %in% c("TRUE","True","true","Yes","yes","1")

# Clean monster types and make capture flags
scoobydoogood_clean <- scoobydoo_good %>%
  separate_rows(monster_type, sep = ",") %>%  # split "TypeA,TypeB" into rows
  mutate(monster_type = str_trim(monster_type), # removes white space from start and end of string
         ...
         captured_scooby = true(captured_scooby), # clean up trues in column
         captured_shaggy = true(captured_shaggy)) %>%  # clean up trues in column
...
captured_plot<-ggplot(captured_data, aes(y = monster_type, x = pct, fill = character)) +
  ...
  labs(title = "Percent Capture by Monster Type",
       subtitle = "Number of Episodes in which Scooby and Shaggy are Captured by Each Monster Type",
  ...
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
        ...
        plot.margin = margin(10, 20, 10, 20))  # breathing room around plot
...
action_data <- scoobydoo_good %>%
  ...
  filter(true(value)) %>% # keep only rows with the action, clean up the TRUE/Yes/1 ast TRUE 
  mutate(char = str_to_title(char),
         type = str_to_title(type)) %>% # use stringr to mutate the character names into capitals
...
(captured_plot + plot_spacer() + action_plot) + # this uses patchwork package
  plot_layout(ncol = 1, heights = c(1, 0.07, 1)) + # adds a gap
  plot_annotation(title = "Scooby Doo (TV Series)",
