...
Quiz:  
If I convert the following factors into a numeric vector, what would you expect to see? data<-factor(c("3","6","NA")) data<-as.numeric(data) what will "data" look like?
c()

```{r}
data<-factor(c("3","6","NA")) 
...
Quiz: 
I am making a barplot and I want to reorder my bars within ggplot to go from highest to smallest value, what function do I use?
...
Quiz:  
I filtered out some values in a column of factors and when I make a barplot all these extra variables show up as empty that are not actually in my dataset. What did I forget to do?

-> use droplevels() after I filtered out the factors
